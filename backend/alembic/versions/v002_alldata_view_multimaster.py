"""update ho.v_alldata — item base da IT01+IT02+IT03 con priorità IT01

Revision ID: v002_alldata_view_multimaster
Revises: v001_alldata_view
Create Date: 2026-04-28
"""
from alembic import op

revision = "v002_alldata_view_multimaster"
down_revision = "v001_alldata_view"
branch_labels = None
depends_on = None
...
WITH
  -- Sessioni correnti per tutte e tre le entity, con priorità IT01 > IT02 > IT03
  _item_sessions AS (
    SELECT id, entity,
  ...
  -- Catalogo master unificato: a parità di item_no vince IT01
  _item_base AS (
  ...
  _promo AS (
    SELECT DISTINCT ON (item_no)
      item_no,
      unit_price AS prezzo_promo,
      last_cost  AS costo_promo
    FROM ho.item_promo
  ...
  _bf AS (
    SELECT DISTINCT ON (item_no)
      item_no,
      unit_price AS prezzo_bf,
      last_cost  AS costo_bf
    FROM ho.item_blackfriday
    WHERE is_active = true
    ORDER BY item_no, created_at DESC
  ),
  _eccezione AS (
  ...
  _bestseller AS (
    SELECT DISTINCT item_no FROM ho.item_bestseller WHERE is_active = true
  ),
  _scrap_inv AS (
    SELECT DISTINCT zebra FROM ho.scrap_inv WHERE is_active = true
  ),
  _scrap_wd AS (
    SELECT DISTINCT zebra FROM ho.scrap_wd WHERE is_active = true
  ),
  _picking AS (
    SELECT DISTINCT item_no FROM ho.item_picking WHERE is_active = true
  ),
  _stock_it01 AS (
    SELECT
      si.item_no,
      si.adm_stock AS adm_it01,
      COALESCE(
        jsonb_object_agg(sd.store_code, sd.quantity)
          FILTER (WHERE sd.store_code IS NOT NULL),
        '{}'::jsonb
      ) AS stock_it01
    FROM ho.stock_items si
    LEFT JOIN ho.stock_store_data sd ON sd.item_id = si.id
    WHERE si.session_id = (
      SELECT id FROM ho.stock_sessions
      WHERE entity = 'IT01' ORDER BY stock_date DESC LIMIT 1
    )
  ...
  _stock_it02 AS (
    ...
    WHERE si.session_id = (
      SELECT id FROM ho.stock_sessions
      WHERE entity = 'IT02' ORDER BY stock_date DESC LIMIT 1
    )
    GROUP BY si.id, si.item_no, si.adm_stock
  ),
  _stock_it03 AS (
    ...
    WHERE si.session_id = (
      SELECT id FROM ho.stock_sessions
      WHERE entity = 'IT03' ORDER BY stock_date DESC LIMIT 1
    )
    GROUP BY si.id, si.item_no, si.adm_stock
  )
SELECT
  ...
  i.description2,
  i.modulo,
  i.model_store_portale,
  i.modulo_numerico,
  i.model_store_portale_num,
  pr.prezzo_promo,
  pr.costo_promo,
  bf.prezzo_bf,
  bf.costo_bf,
  ec.eccezione_prezzo_1,
  ec.eccezione_prezzo_2,
  ec.eccezione_sconto,
  ec.eccezione_testo,
  ec.eccezione_testo2,
  ec.eccezione_categoria,
  ec.eccezione_tipo,
...
# Downgrade ricrea la v001 (solo IT01 come master)
_VIEW_SQL_V001 = """
CREATE OR REPLACE VIEW ho.v_alldata AS
WITH
  _item_session AS (
  ...
  _promo AS (
    SELECT DISTINCT ON (item_no) item_no,
      unit_price AS prezzo_promo, last_cost AS costo_promo
    FROM ho.item_promo WHERE is_active = true ORDER BY item_no, created_at DESC
  ),
  _bf AS (
    SELECT DISTINCT ON (item_no) item_no,
      unit_price AS prezzo_bf, last_cost AS costo_bf
    FROM ho.item_blackfriday WHERE is_active = true ORDER BY item_no, created_at DESC
  ),
  _eccezione AS (
    SELECT DISTINCT ON (zebra) zebra,
      prezzo_1 AS eccezione_prezzo_1, prezzo_2 AS eccezione_prezzo_2,
      sconto AS eccezione_sconto, testo_prezzo AS eccezione_testo,
      testo_prezzo2 AS eccezione_testo2, categoria AS eccezione_categoria,
      eccezione AS eccezione_tipo
    FROM ho.eccezioni WHERE is_active = true ORDER BY zebra, created_at DESC
  ),
  ...
  _stock_it01 AS (
    SELECT si.item_no, si.adm_stock AS adm_it01,
      COALESCE(jsonb_object_agg(sd.store_code, sd.quantity) FILTER (WHERE sd.store_code IS NOT NULL), '{}'::jsonb) AS stock_it01
    FROM ho.stock_items si LEFT JOIN ho.stock_store_data sd ON sd.item_id = si.id
    WHERE si.session_id = (SELECT id FROM ho.stock_sessions WHERE entity='IT01' ORDER BY stock_date DESC LIMIT 1)
    GROUP BY si.id, si.item_no, si.adm_stock
  ),
  _stock_it02 AS (
    SELECT si.item_no, si.adm_stock AS adm_it02,
      COALESCE(jsonb_object_agg(sd.store_code, sd.quantity) FILTER (WHERE sd.store_code IS NOT NULL), '{}'::jsonb) AS stock_it02
    FROM ho.stock_items si LEFT JOIN ho.stock_store_data sd ON sd.item_id = si.id
    WHERE si.session_id = (SELECT id FROM ho.stock_sessions WHERE entity='IT02' ORDER BY stock_date DESC LIMIT 1)
    GROUP BY si.id, si.item_no, si.adm_stock
  ),
  _stock_it03 AS (
    SELECT si.item_no, si.adm_stock AS adm_it03,
      COALESCE(jsonb_object_agg(sd.store_code, sd.quantity) FILTER (WHERE sd.store_code IS NOT NULL), '{}'::jsonb) AS stock_it03
    FROM ho.stock_items si LEFT JOIN ho.stock_store_data sd ON sd.item_id = si.id
    WHERE si.session_id = (SELECT id FROM ho.stock_sessions WHERE entity='IT03' ORDER BY stock_date DESC LIMIT 1)
    GROUP BY si.id, si.item_no, si.adm_stock
  )
SELECT i.item_no, i.description, i.description_local, i.warehouse, i.last_cost, i.unit_price,
  ...
  COALESCE(s3.stock_it03, '{}'::jsonb) AS stock_it03
FROM ho.item_master_it01 i
JOIN _item_session cs ON i.session_id = cs.id
