"""kgl_list: aggiungi colonna kgl_l (col G KG/L) e aggiorna ho.v_alldata

Revision ID: v004_kgl_add_kgl_l
Revises: v003_alldata_view_sync_tables
Create Date: 2026-04-29
"""
from alembic import op

revision = "v004_kgl_add_kgl_l"
down_revision = "v003_alldata_view_sync_tables"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute("ALTER TABLE ho.kgl_list ADD COLUMN IF NOT EXISTS kgl_l NUMERIC(10, 4)")
...
def downgrade() -> None:
    op.execute("DROP VIEW IF EXISTS ho.v_alldata")
    op.execute("ALTER TABLE ho.kgl_list DROP COLUMN IF EXISTS kgl_l")
