...
const HEADERS = ["ZEBRA", "BLOCCATO", "DESCRIZIONE", "categoria"]
const HEADER_TO_FIELD = {
  "ZEBRA": "zebra", "BLOCCATO": "bloccato",
  "DESCRIZIONE": "descrizione", "CATEGORIA": "categoria",
}

const EMPTY_ROW = { zebra: "", bloccato: "BLOCCATO", descrizione: "", categoria: "" }

export default function ScrapWdPanel() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["items-scrap-wd"],
    queryFn: () => itemsApi.getScrapWd().then(r => r.data),
    staleTime: 30_000,
  })

  const rows = data?.rows ?? []

  function refresh() { queryClient.invalidateQueries({ queryKey: ["items-scrap-wd"] }) }
  ...
  async function handleDelete() {
    if (!selected.size || !confirm(`Eliminare ${selected.size} righe da SCRAP WD?`)) return
    setSaving(true)
    try { await itemsApi.deleteScrapWd([...selected]); setSelected(new Set()); setMode(null); refresh() }
  ...
  async function handleSaveEdit() {
    ...
    setSaving(true)
    try { await itemsApi.updateScrapWd(editingId, editRow); setEditingId(null); setEditRow(null); refresh() }
    finally { setSaving(false) }
  }
  ...
  async function handleSaveNew() {
    ...
    setSaving(true)
    try { await itemsApi.appendScrapWd([newRow]); setNewRow(null); setLookupStatus(null); refresh() }
    finally { setSaving(false) }
  }
  ...
  function handleImportList(e) {
    ...
    reader.onload = async (evt) => {
      ...
      const ws = wb.Sheets[wb.SheetNames.find(n => n.toUpperCase().includes("SCRAP") || n.toUpperCase().includes("WD")) || wb.SheetNames[0]]
      ...
      if (parsed.length) { await itemsApi.appendScrapWd(parsed); refresh() }
    }
  ...
  function handleDownload() {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([HEADERS, ...rows.map(i => COLUMNS.map(k => i[k] ?? ""))]), "SCRAP WD")
    XLSX.writeFile(wb, "ScrapWd.xlsx")
  }

  async function handleReplace() {
    if (!importedRows) return
    if (!confirm("Sei sicuro? Questa operazione sostituira i dati nella tabella SCRAP WD.")) return
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12 gap-2 text-gray-400">
        ...
        <span className="text-sm">Caricamento SCRAP WD...</span>
      </div>
    )
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3 flex-wrap justify-between">
        <div className="flex items-center gap-2 text-xs">
          <span className="text-gray-400 font-semibold uppercase text-[9px] tracking-wider">Scrap Wd</span>
      ...
      <div className="flex items-center gap-1.5">
        <span className="text-sm font-bold text-gray-700">SCRAP WD</span>
      ...
      <div className="relative">
        <label htmlFor="scrap-wd-search" className="sr-only">Cerca scrap wd</label>
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" aria-hidden="true" />
        <input id="scrap-wd-search" type="text" placeholder="Cerca..." value={search} onChange={e => setSearch(e.target.value)}
