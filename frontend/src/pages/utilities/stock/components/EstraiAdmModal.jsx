...
export default function EstraiAdmModal({ onClose }) {
  ...
  async function handleGenera() {
    ...
    try {
      const response = await stockApi.admExtract(stockDate, (evt) => {
      ...
      const filename = `${stockDate.replace(/-/g, "")} ADM OneItaly.xlsx`
  ...
  function handleSendEmail() {
    ...
    const subject = encodeURIComponent(`${stockDate.replace(/-/g, "")} ADM OneItaly fonte NAV`)
    const body = encodeURIComponent(`In allegato il file ADM OneItaly del ${displayDate}.\n\nFile: ${filename} (${formatBytes(blob.size)})`)
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          <div>
            <h3 className="text-base font-bold text-gray-800">Estrai ADM</h3>
            <p className="text-xs text-gray-400 mt-0.5">Excel con 3 fogli — ADM IT01 · ADM IT02 · ADM IT03</p>
          </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span className="flex items-center gap-1.5">
                  ...
                  {progress.pct !== null ? "Download in corso..." : "Generazione Excel..."}
                </span>
