...
function YesterdayTable({ preview }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
      <div className="bg-violet-500 px-4 py-2 flex items-center justify-between">
        <span className="text-white font-bold">IT03 — {col}</span>
...
export default function SalesIT03() {
  const navigate = useNavigate()
  const [tsv, setTsv] = useState("")
  const preview = useSalesCheckStore((s) => s.it03)
  const setPreview = useSalesCheckStore((s) => s.setPreview)

  const parseMut = useMutation({
    mutationFn: (raw) => salesApi.parseSalesData({ raw_tsv_it03: raw }),
    onSuccess: (res) => setPreview("it03", res.data.it03),
  })
  ...
  function resetPreview() {
    setTsv("")
    setPreview("it03", null)
  }

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800">Vendite IT03</h1>
      ...
      {!preview ? (
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm space-y-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold px-2 py-0.5 rounded bg-violet-100 text-violet-700">IT03</span>
          ...
          <label htmlFor="tsv-it03" className="sr-only">Dati TSV IT03</label>
          <textarea
            id="tsv-it03"
            value={tsv}
            onChange={handlePaste}
            placeholder="Incolla qui i dati IT03 copiati da Navision (Ctrl+V)..."
            rows={6}
            className="w-full text-xs font-mono bg-gray-50 border-2 border-violet-400 rounded-lg px-3 py-2 outline-none resize-y focus:ring-2 focus:ring-violet-300"
          />
