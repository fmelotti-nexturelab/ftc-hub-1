    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
      ...
      {showAdd && (
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200 grid grid-cols-2 gap-3">
          <input placeholder="Codice store *" value={newStore.store_code}
          ...
          <input placeholder="Nome store" value={newStore.store_name}
            onChange={(e) => setNewStore((s) => ({ ...s, store_name: e.target.value }))}
            className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-300 outline-none" />
          <select value={newStore.reason} onChange={(e) => setNewStore((s) => ({ ...s, reason: e.target.value }))}
            className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-blue-300 outline-none">
            {REASONS.map((r) => <option key={r}>{r}</option>)}
          </select>
          <input placeholder="Note (opzionale)" value={newStore.notes}
